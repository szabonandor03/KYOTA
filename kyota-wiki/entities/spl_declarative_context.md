...
3. Context gathering and answer generation are separate phases. First `SELECT` the minimum evidence set. Then `GENERATE` the output, patch, or plan.
4. Treat tool schemas, prompt fragments, and invariants as selectable context artifacts, not as permanent prompt furniture.
5. JIT-load only the smallest viable tool and schema set for the current execution tick, then unload it after the task reaches a terminal state.
6. Do not generate while still browsing broadly. Unbounded reading causes context creep and weakens auditability.
7. If the selected evidence exceeds the budget, reduce scope before proceeding. Do not keep adding files and hope the model sorts it out.
8. When a task is high-stakes or complex, include a brief `EXPLAIN` note describing which sources were selected, which execution loop was chosen, and why they fit the budget.
